...
def plot_confusion_matrix(cm, classes,max_val = 300,
                          normalize=False,
                          title='Confusion matrix',
                          cmap=plt.cm.Blues,cb=False):
    """
    This function prints and plots the confusion matrix.
    Normalization can be applied by setting `normalize=True`.
    """
    cm_clip = copy.deepcopy(cm)
    cm_clip[cm>max_val] = max_val 
    plt.imshow(cm_clip, interpolation='nearest', cmap=cmap)
    plt.title(title)
    if cb: plt.colorbar()
